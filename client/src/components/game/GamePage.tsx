...
export default function GamePage({ initialLobby }: { initialLobby: Game }) {
  ...
  const [chatMessages, setChatMessages] = useState<Message[]>([
    {
      author: {},
      message: `Добро пожаловать! Вы можете пригласить друзей, чтобы они могли наблюдать или играть по ссылке. Наслаждайтесь!`
    }
  ]);
  ...
  function updateTurnTitle() {
    ...
    if (!lobby.endReason && lobby.side === lobby.actualGame.turn()) {
      document.title = "(твоя очередь) ton-chess";
    } else {
      document.title = "ton-chess";
    }
  }
  ...
  function getPlayerHtml(side: "top" | "bottom") {
    const blackHtml = (
      <div className="flex w-full flex-col justify-center">
        ...
        >
          {lobby.black?.name || "(игрок не подключился)"}
        </a>
      ...
      <div className="flex w-full flex-col justify-center">
        ...
        <span className="flex items-center gap-1 text-xs">
          ...
          {lobby.white?.connected === false && (
            <span className="badge badge-xs badge-error">отключён</span>
          )}
        </span>
      </div>
    );
        ...
        <div className="mb-auto flex w-full p-2">
          ...
          <div className="flex flex-1 flex-col gap-1">
            <div className="mb-2 flex w-full flex-col items-end gap-1">
              {lobby.endReason ? "Archived link:" : "Пригласите друзей:"}
              <div
                className={
            ...
            <div className="bg-neutral absolute w-full rounded-t-lg bg-opacity-95 p-2">
              {lobby.endReason ? (
                ...
                <div className="flex flex-wrap items-center justify-center gap-2">
                  <span>Ваш опоннет был отключён.</span>
